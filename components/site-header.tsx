    ...
    if ((item.security === 'public' || hasPermission(role, item)) && token != '') {
      return (
        <Link
          ...
          onClick={toggleMenu}
          className={cn(
            `flex items-center rounded border-b p-2 text-sm font-medium text-muted-foreground hover:bg-gray-200 md:border-0 md:text-white  md:hover:bg-white md:hover:text-black lg:border-0${token == '' && 'hidden'}`,
          )}
        >
          {item.title}
        </Link>
  ...
  return (
    ...
    <header className="sticky top-0 z-40 w-full  bg-black shadow" id="noprint">
      {(token || pathName == '/login') &&
        <nav className="container flex h-16 items-center space-x-4 text-muted-foreground sm:justify-between sm:space-x-0 md:text-white">
                    ...
                    (
                      ...
                      <DropdownMenu key={index}>
                        <DropdownMenuTrigger className={cn(
                          `flex items-center rounded border-b px-2 py-4 text-sm font-medium text-muted-foreground hover:bg-white hover:text-black   md:border-0 md:text-white lg:border-0 ${token == '' && 'hidden'}`,
